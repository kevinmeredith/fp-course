...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f b
  (<$>) f x = StateT $ \s -> 
       let result = runStateT x s
       in (<$>) (\(a, s1) -> (f a, s1)) result 
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) ::
   StateT s f (a -> b)
    ...
    -> StateT s f b
  (<*>) fn fa = StateT $ \s -> 
     runStateT fn s >>= (\(func, s2) ->
      runStateT fa s2 >>= (\(a, s3) -> 
        pure (func a, s3)
        )
      ) 
...
instance Functor f => Functor (OptionalT f) where
  (<$>) f fa = OptionalT $ (\opt -> f <$> opt) <$> (runOptionalT fa)
...
instance Monad f => Monad (OptionalT f) where
  (=<<) f fa = OptionalT $ (runOptionalT fa) >>= (\mFa ->
                case mFa of
                  Empty -> pure Empty 
...
instance Applicative (Logger l) where
  pure = Logger Nil
  (<*>) (Logger list1 f) (Logger list2 a) =
    Logger (list1 ++ list2) (f a)
